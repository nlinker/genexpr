...
incPtr :: Ptr Word8 -> Ptr Word8
incPtr = (`advancePtr` 1)

decPtr :: Ptr Word8 -> Ptr Word8
decPtr = (`advancePtr` (-1))

flop :: Int -> Perm -> IO ()
flop k xs = flopp xs (xs `advancePtr` k)
  where
    flopp i j = when (i < j) $ swap i j >> flopp (incPtr i) (decPtr j)
    swap i j = do
      a <- peek i
      b <- peek j
      poke j a
...
main = do
   ...
   vars <- forM [0,bk..fact-1] $ \ ix -> do
     var <- newEmptyMVar
     _ <- forkIO (unrank ix n $ \ p -> genPermutations n ix (min fact (ix + bk)) p >=> putMVar var)
     return var
